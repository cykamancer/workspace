...
int load_census(FILE *census_file, Table *database)
{
	int error_occurred = 0;
	...
	while (!error_occurred && !feof(census_file)) {
		...
		if (l == line){
		    ...
		    lineptr=strtok(line, " ");
		    entry->value=(char*)malloc(strlen(lineptr)+1);
...
void logger(int logging, FILE *file, char *message)
{
	...
	if (logging == 2) {
		//printf("In logging == 2 option\n"); //delete this
		//printf(message); //delete this
		//printf("Past printf message\n"); //delete this
